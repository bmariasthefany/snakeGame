$(document).ready(function load() {
    const snakeBoard = $("#snakeBoard").get(0);
    ...
    function changeDirection(event) {
        ...
        if (((keyPressed === upKey) || (keyPressed === arrowUp)) && !goingDown) {
            dx = 0;
            dy = -10;
        }
        if (((keyPressed === downKey) || (keyPressed === arrowDown)) && !goingUp) {
            dx = 0;
            dy = 10;
        }
        if (((keyPressed === rightKey) || (keyPressed === arrowRight)) && !goingLeft) {
            dx = 10;
            dy = 0;
        }
        if (((keyPressed === leftKey) || (keyPressed === arrowLeft)) && !goingRight) {
            dx = -10;
            dy = 0;
        }
    ...
    function gameEnded() {
        ...
        else if ($("#infinite").is(':checked') === true) {
